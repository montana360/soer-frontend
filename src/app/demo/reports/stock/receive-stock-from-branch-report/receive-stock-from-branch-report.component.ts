...
@Component({
  selector: 'app-receive-stock-from-branch-report',
  templateUrl: './receive-stock-from-branch-report.component.html',
  styleUrls: ['./receive-stock-from-branch-report.component.scss']
})
export class ReceiveStockFromBranchReportComponent implements OnInit {
  ...
  ngOnInit() {
    this.getBranches();
    this.getReceiveStockBranchReport();
  ...
  getReceiveStockBranchReport(){
    // receive_stock_report
    this.auth.store('receive_stock_from_branch_report', this.data).subscribe(
      response => {
